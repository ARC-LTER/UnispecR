...
## Load Data
You can choose to select unispec data files interactively; however, it is usally more convenient to let R search: `r data_path` -- for all `.spu` files. 
...
ref_choice <- df %>% 
  ...
  select(Date, Site, Block, Weather, Notes, Measurement, FileNum, filename) %>% 
  distinct() %>% 
