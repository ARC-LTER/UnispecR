...
data_path <- rchoose.dir(caption = "Select Unispec files directory")
  ...
  data_path  <- "UnispecRecord/UnispecData/" # local computer
  dir_year <- "2017"
...
### Load multispec data
multispec_data <- read_rds(paste0(data_path, dir_year, "_multispec_data.rds"))
...
### Load File Key Fix
```{r}
unispec_file_key <- read_csv(paste0(data_path, dir_year, "_unispec_file_key.csv"), col_types = cols(
...
## Summary Table
df %>% group_by(Date) %>% 
  summarize(Sites = str_c(unique(Site), collapse = ", "), FileNum = n())
```
...
check_site <- "NANT"
check_date <- "2017-08-10" # necessary to unlist dates vector
...
## SPECIFY SITE/DATE/ETC to ZOOM IN ON
# check_site <- "MAT"
# check_date <- "2017-06-08" # necessary to unlist dates vector
