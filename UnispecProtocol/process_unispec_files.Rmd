...
## WHEN APPENDING DATA: filter to only read some sites / dates to add to existing dataframe
spu_file_subset <- spu_files[str_detect(spu_files, "2019")]
...
spu_metadata <- map_dfr(spu_file_subset, read_spu_file_metadata) %>% 
  mutate(spu_filename_full = spu_file_subset)

spu_data_all <- spu_metadata %>% 
  mutate(Spectra=map(spu_filename_full, function(x) read_spu_file_spectra(x)))

spu_dataframe <- spu_data_all %>% select(spu_filename, Site, FileNum, Date, DateTime, Integration_ms, Temp, Remarks, Spectra, everything())
...
spu_dataframe_new <- anti_join(spu_dataframe, spu_dataframe_from_file %>% select(spu_filename)) 
...
spu_dataframe_full <- bind_rows(spu_dataframe_from_file, spu_dataframe_new) %>% 
  arrange(DateTime)

## Rename 
spu_dataframe <- spu_dataframe_full
...
### Read File Key & Join with .spu data
```{r}

file_key <- paste0(data_path, "2019_unispec_file_key.csv")

unispec_file_key <- read_csv(file_key, skip_empty_rows = T, col_types = cols(
...
unispec_file_key_fix <- df %>% 
  select(spu_filename, Date, Site, Block, Treatment, Replicate, FileNum, Notes, Weather, Type) %>% 
  mutate(key_fix = NA)


fix_keyname <- paste0(data_path, dir_year,"_unispec_file_key_fix.csv")
write_csv(unispec_file_key_fix, path = fix_keyname)
```
...
time_check <- meta_timedata %>% select(Site, Date, DateTime, FileNum, Block, Treatment, Replicate, diff, Integration_ms) %>% ungroup()
