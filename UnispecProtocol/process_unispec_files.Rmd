...
check_site <- "HIST"
check_date <- "2019-06-22" # necessary to unlist dates vector
...
time_check <- meta_timedata %>% select(Site, Date, DateTime, FileNum, Block, Treatment, Replicate, diff, Integration_ms, Type) %>% ungroup()

# Examine dataframe
time_check %>% print(n=250)
```
...
ref_data %>% filter(Spectra %>% map(is.null) %>% map_lgl(any))
