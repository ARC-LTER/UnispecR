...
data_path <- "/home/ruby/UnispecData/2020/2020-07-19/"
field_keys <- list.files(path=data_path, pattern="*key.csv", full.names=TRUE)
...
field_key <- field_keys %>%  purrr::map(function(file_name) read_csv(file_name)) %>% 
  reduce(rbind) %>% 
  mutate(Date = date(params$session_date)) # Add date

# Confirm spu_dataframe has same Site abbreviation as key
if(unique(spu_dataframe$Site) %>% sort() != unique(field_keys$Site) %>% sort()) {
...
df %>% 
  ...
  facet_grid(ref_part ~ Channel, scales="free") 
