...
if (!file.exists(params$function_file)) {
  params$function_file<-rchoose.files(caption = "Select file:unispec_protocol_functions.R")
  }
source(params$function_file) # file loads required functions
```
...
read_spu_file_metadata <- function(filename) {
  ...
  # OUTPUT: dataframe with 15 columns on instrument status, scan settings, max/min value
    ...
    ScanType <- str_split(Remarks[7], pattern = "=")[[1]][2] 
    ...
    # Line 6-9: 
    Minimum <- str_extract(text[6], "\\d+.+\\d") # Walvelength, ChB min
    MinValue_nm <- str_split(Minimum, boundary("word"))[[1]][1] # Wavelength
    MinValue <- str_split(Minimum, boundary("word"))[[1]][2] # ChB AD
    
    Maximum <- str_extract(text[7], "\\d+.+\\d") # Walvelength, ChB max
    MaxValue <- str_split(Maximum, boundary("word"))[[1]][2]
    MaxValue_nm <- str_split(Maximum, boundary("word"))[[1]][1]
    ...
    # Metadata 
    metadata <- tibble(spu_filename, DateTime, FileNum, ScanType, Integration, NumberScans,MinValue_nm, MinValue, MaxValue_nm, MaxValue, Limits, Temperature, Battery, Aux, DarkscanID, Remarks)
...
spu_folder <- file.path(data_path,params$session_date)
...
field_keys <- read_csv(field_key, skip_empty_rows = T, col_types = "Dcccdcdcc") %>% ## remove NA rows 
  filter(Date == params$session_date)
...
absurd_raw_data <- df %>% 
  ...
  nest(Spectra = c(Wavelength, ChA, ChB, Reflectance))
