...
## Required R Packages and R files
...
## Data directories, date and key files
  ...
  * Base folder where the yearly files are stored. Should be OneDrive. The structure is year/spu_orig/yyyy-mm-dd  where yyyy-mm-dd is sample date.
  
  * Key file with Date	Site	Block	Treatment 	Replicate	Location 	FileNum	Notes	Weather
...
data_path <-file.path(getwd(),year(session_date))

## Check if directory exist
if (!dir.exists(data_path)) {data_path <- rchoose.dir(caption = "Select Unispec Data directory")}
...
## Field key file 
field_key <- file.path(data_path,paste0(year(session_date),"_unispec_field_key.csv"))

## Check if file exist
if (!file.exists(field_key)) {data_path <- rchoose.files(caption = "Select field key file")}
...
### Read Field Key 
Note that the format of the Date in the csv key file needs to be yyyy-mm-dd

```{r}

# field_key_files <- list.files(path = data_path, pattern = "field_key.csv$", full.names = T, recursive=T)
# 
# field_keys <- map_dfr(field_key_files, read_csv)
# 
# 
# ## quick cheat: use full field key for 2019
# field_key <- "C:\\Users\\jl01596\\Documents\\UnispecData\\2019/2019_unispec_field_key.csv"

# field keys for this session
...
View(dim_data)
message("Primarily Darkscans should show up.")
```
