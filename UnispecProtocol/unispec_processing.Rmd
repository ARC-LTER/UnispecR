...
This is a R notebook. Code chucks can be run individually and Previewed
or Run all the chucks and then check the output for errors or success.
...
-   Field Key file with header
    ...
    File name convention is year_unispec_field_key.csv
...
if(interactive()){
  data_path <- selectDirectory(caption = "Select directory containing .spu files")
} else 
...
extra_files <- anti_join(spu_dataframe, field_keys,by = c("Date", "FileNum", "Site")) %>% pull(spu_filename)
...
```{r maxed, dependson="raw_data", fig.show="hold", out.width="50%"}
