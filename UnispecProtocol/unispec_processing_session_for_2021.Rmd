...
(ref_plot <- ggplot(full_join(mean_refs, single_refs),
  ...
  geom_line(aes(color=factor(Integration), alpha = factor(num_files)))) 
...
raw_data_with_refs <- raw_data %>% 
  filter(!str_detect(Treatment, "REF|DARK|THROWAWAY")) %>% # remove non-data (ref, dark, throwaway) scans
  unnest(Spectra) %>% 
  filter(Wavelength > 400, Wavelength < 1000) %>% # remove edge wavelengths, because instrument unreliable at extreme wavelengths
