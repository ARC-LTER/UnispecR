...
unispec_file_key <- df %>% 
  select(spu_filename, DateTime, Date, Site, Block, Treatment, ScanRep, ScanLocation, Plot.x, Plot.y, FileNum, Notes, Weather, Type) %>% 
  distinct(spu_filename, .keep_all = T)


fix_keyname <- paste0(data_path, dir_year,"_processed_unispec_file_key.csv")
...
file_key_fix <- paste0(data_path, "2019_processed_unispec_file_key.csv")
...
check_site <- "HIST"
check_date <- "2019-07-02" # necessary to unlist dates vector
...
time_check <- meta_timedata %>% select(Site, Date, DateTime, Block, Treatment, FileNum, ScanLocation, Plot.y, diff, Integration_ms, Type) %>% ungroup()
...
ref_data <- ref_keys %>% select(Date, Site, Site_REF, FileNum, Block, Treatment, ScanNum, Weather, Notes) %>% 
...
## PLOTS
ref_plot_all + ggtitle("All 2019 White Reference Scans")
...
### subset spectra to usable wavelengths (400-1000nm)
...
df_ref <- left_join(spu_for_plots, ref_summary) %>% 
  select(Date, DateTime, Site, Block, Treatment, int_REF, Wavelength, ChB, ChA, CorrectionFactor,  ScanNum, spu_filename, FileNum, Integration_ms, Weather, Notes, ChB_REF, ChA_REF, Notes_REF, ref_filenames, everything()) %>% 
  
  # drop_unneccessary column
  select( -Reflectance) %>% 
...
df_subset <- df_corrected %>% 
  filter(Date == "2019-08-10", Site == "HIST") %>%
  slice(1:100) %>% 
...
ggplot(data = df_subset, mapping = aes(x = Wavelength, y = Reflectance )) + 
  geom_line(aes(color = spu_filename, linetype = Status)) + 
    facet_wrap(vars(Date, Site, Block, Treatment))
```
...
index_data <- read_csv(paste0(data_path, "2019_processed_veg-indices.csv")) %>% 
  ...
  left_join(unispec_file_key_fix)


index_data <- read_rds("/home/ruby/UnispecR/UnispecRecord/UnispecData/2019/2019_index_data.rds") 
```

## Calculate INDICES for new spu files
Calculates for all files not in existing dataframe of index_data
```{r}
## identify new files by filename list 
...
df_processed_new <- anti_join(processed_spectra, index_data %>% select(spu_filename)) 
...
indices_all <- index_data %>% 
  ...
  # select(spu_filename, DateTime, Date, Site, Block, Treatment, ScanNum, ScanLocation, FileNum, Notes, Weather, Type, ref_filenames, Spectra, Indices) %>% 
   bind_rows(indices_new_formatted) %>% 
  # 
...
## SPECIFY SITE/DATE/ETC to ZOOM IN ON
check_site <- "LMAT"
check_dates <-  c(lubridate::ymd("2019-06-01"), lubridate::ymd("2019-08-22")) # necessary to unlist dates vector
CT <- c("CT1", "CT2", "CT")
check_treatments <- c(CT, "F0.5", "F1", "F2", "F5", "F10")
...
ggplot(data = index_subset, aes(x=Date,y=index)) +
  geom_point(aes(color=Block, shape=factor(ScanRep)))
