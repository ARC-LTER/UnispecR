...
```{r load_data}
load("HistoricUnispecData/2016-unispec_data.Rda")
unispec_data_2016 <- tbl_df(unispec_data)
```

Convert dataframes from Jim format to Ruby format to read into shiny app. 
```{r convert_data, dependson="load_data"}

### RESAVED DATA CHECKS
names(unispec_data)
unispec_data %>% filter(is.na(ChAmax))

## Check FileNum.x == FileNum.y for all rows 
unispec_data %>% filter(FileNum.x != FileNum.y)

## Check Site.x == Site.y for all rows 
unispec_data%>% filter(Site.x != Site.y)

## Check for missing spu_filename 
unispec_data %>% filter(is.na(spu_filename))

## Check for large filenumbers 
### Inconsistent File Number reading due to number at end of site names 
large_filenums <- unispec_data_2016 %>% 
  filter(FileNum.x > 500) %>%  # Sites w/numbers at the end mis-read in as part of file number. All > 100000: Unispec-DC measurements only go up to 5 digits. 
  select(Date, Site.x, Block, key_num.x, FileNum.x, FileNum.y, Spufilename.x, Spufilename.y, Spu_filename)
large_filenums
...
data_2016 <- unispec_data_2016 %>% 
  ...
  select(-c(ReflecMean, ReflecSERR, Number_ChAmax)) %>%  # not necessary, calculated later using Markdown processing 
  select(-c(key_num.x, key_num.y, key)) %>% # redundant info to FileNum
  select(-c(Spufilename.y, Spufilename.x)) %>% # redundant on Spu_filename
  ## Rename columns
  ...
  filter(!Type %in% c("Throwawayscan", "Darkscan", "not_in_process_file")) %>% # remove non-data or reference files 
  ## Rename rows 
  mutate(FileNum = FileNum %% 100000) # correct for large filenumbers (>5 digits due to number at end of Site name)

data_2016 %>% filter(Site == "WSG") %>% filter(Date %in% c("2016-07-12", "2016-07-13"))

tidy_2016 <- data_2016 %>% # nest index data 
  ...
  inner_join(data_2016 %>% select(-c('NDVI(MODIS)':'ChlIndex')))  # rejoin w/Spectra column 

##rename for future chunks 
tidy_df <- tidy_2016
...
## Useful vectors for standardizing treatment names
trtmt_list_df <- tidy_df %>% select(Treatment) %>% unique() %>% pull() # all treatments 

## Look for weird things
tidy_df %>% select(-Spectra, -Indices) %>% 
  summary()
...
tidy_df <- tidy_df %>% 
  ...
  mutate(Site = replace(Site, Site %in% DHT, "DHT")) %>% 
...
```{r maxed_data_files, dependson="convert_data"} 
maxed_data_files <- tidy_df %>% filter(ChAmax > 65000 | ChBmax > 65000) # list of files w/max'd spectra
...
## Plot max'd spectra 
first_file <- 121 # choose subset of files to plot 
last_file <- 140 # > 20 is really slow / scrunched
...
last_file <- 16 # avoid plotting >50 at a time

## Select a subset to plot 
ref_subset <- ref_data_files %>% 
  slice(first_file:last_file) %>% 
  unnest(Spectra) %>% 
  mutate(REF_CorrectionFactor = ChA/ChB) %>% 
  gather(key = Channel, value = Intensity, ChB, ChA) %>% 
...
ref_plot <- ggplot(data = ref_subset, mapping = aes(x = Wavelength, y = CorrectionFactor_Intensity)) +
  geom_line(aes(color=Replicate, linetype=Channel)) +
...
#   geom_line(aes(color=Replicate, linetype=factor(int))) +
#   facet_wrap(vars(Date,Site_REF, Block), ncol=4)


## Output Table of Chosen References
ref_choice <- ref_data %>% 
  select(Date, Site, Site_REF, FileNum, Block, Replicate, Weather, Notes, filename) %>% 
  distinct() %>% 
...
df_ref <- inner_join(df_clean, ref_summary) %>% 
  select(Date, Time, Site, Block, Treatment, Replicate, Wavelength, int, int_REF, ChB, ChA, ChB_REF, ChA_REF, CorrectionFactor_REF, Weather, Notes, Notes_REF, FileNames_REF, filename, FileNum, keyname) %>%
